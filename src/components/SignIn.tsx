...
const SignIn = ({ calbackUrl = "/" }: SignInProps) => {
  ...
  return (
    <form onSubmit={handleSignIn} className="w-full h-full p-24 flex my-auto">
      <section className="w-1/2 flex flex-col justify-start items-start gap-8 bg-white/5 outline outline-white/10 outline-1 p-10 rounded-xl">
        ...
        <p className="text-muted-foreground font-mono font-bold">
          Sign in to your <b className="text-primary"> LegalGuide AI </b> account
        </p>
        <button
          disabled={loading}
          className={`flex justify-evenly items-center gap-4 ${
            loading
              ? " bg-popover cursor-wait"
              : "bg-black cursor-pointer"
          }  text-primary font-medium px-6 py-2 raised  transition-all`}
        >
